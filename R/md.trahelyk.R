...
# cd <- function(x) {
#   setwd(paste0("/private/tmp/hartkyVolumes/OHSU/SOM/Surgery/RESEARCH/Admin - Research/Statistics/khart_projects/", x))
# }
# 
# 
...
md.log.summary <- function(mdl, rownames=NULL, caption="", d=2, latex=TRUE, label="", asymp.nml = FALSE) {
  if(is.null(rownames)) rownames = names(mdl$coef)[2:length(mdl$coef)]
  outt <- data.frame(matrix(vector(), 
                            0, 3, 
                            dimnames=list(c(), 
